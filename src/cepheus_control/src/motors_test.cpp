...
void lsPosCallback(const std_msgs::Float64::ConstPtr& cmd) {
	...
	// 	return;
	// else
	q1 = -(cmd->data) + offsetq1; //o encoder tou mou ta diabazei anapoda
}
...
void rePosCallback(const std_msgs::Float64::ConstPtr& cmd) {
	...
	// 	return;
	// else
	q3 = -(cmd->data) + offsetq3;  //o encoder tou karpou ta diavazei anapoda
}
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber base_pos_sub = nh.subscribe("/vicon/cepheusbase/cepheusbase", 1, base_posCallback); //TO ANOIGOKLEINO
    ...
    while(!legitChar){
        ...
        if(cmd == 'Y'){
            ...
            ros::spinOnce();
            theta0des = theta0;  //to arxiko theta0, thelo na meinei statheri gonia
    ...
    while(ros::ok() && !shutdown_requested){
        ...
        if(initialiseArm){
            ...
            prev_torq[1] = torq[1];
            prev_torq[2] = torq[2];
            torq[0] = 25*errorq[0] + 2.5*errorqdot[0]; //apo emena afta
            torq[1] = -(58.42645738*errorq[1] + 1.047501073*errorqdot[1])/186; //- giati ta mesa einai pros ta arnhtika , to /186 to vlepo pantou ston vraxiona
            torq[2] = (40.54481*errorq[2] + 1.23743613*errorqdot[2])/186;
            torq[3] = -(30.90236545*errorq[3] + 1.089994475*errorqdot[3])/186;

            // if(torq[0] > rw_max_torque){ sxolio
            //     torq[0] = rw_max_torque;
            // }
            // if(torq[0] < -rw_max_torque){
            //     torq[0] = -rw_max_torque;
            // }
