...
if len(sys.argv) < 2:
    print("Usage: python3 plot_torque_vs_xeethetadot.py <bag_file>")
    sys.exit()
...
with rosbag.Bag(bag_file, 'r') as bag:
    for topic, msg, t in bag.read_messages(topics=['/cepheus/left_wrist_torque', '/cepheus/xeethetadot']):
        ...
        elif topic == '/cepheus/xeethetadot':
            xeethetadot.append(msg.data)
            print(f"Read xeethetadot: {msg.data} at time {t.to_sec()}")

# Ensure both lists have the same number of elements
min_length = min(len(torques), len(xeethetadot))
torques = torques[:min_length]
xeethetadot = xeethetadot[:min_length]
...
print(f"Number of xeethetadots: {len(xeethetadot)}")
...
if len(time_stamps) > 0:
    ...
    # Plot xeethetadot
    plt.plot(time_stamps, xeethetadot, label='/xeethetadot', color='b')
