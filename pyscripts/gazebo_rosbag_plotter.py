...
def forcesmooth(rawlist, start,stop,fs):
    for i in range(int(start*fs),int(stop*fs)):
        if(abs(rawlist[i])<0.001):
            rawlist[i] = rawlist[i-1]-0.0015
        rawlist[i] = rawlist[i] - 0.002
    for i in range(200):
        rawlist[i] = 0
    for i in range(len(rawlist)):
        rawlist[i] = abs(rawlist[i])
    return rawlist
...
def smooth_contact_force(fext_x, fs=200, t_start=50, t_end=60, target_force=-0.5):
    """
    Smoothly rounds fext_x to target_force in the given time window.
    
    Parameters:
        fext_x (list or np.array): Original force time series.
        fs (int): Sampling frequency (default 200 Hz).
        t_start (float): Start time for rounding (seconds).
        t_end (float): End time for rounding (seconds).
        target_force (float): Target force value.
    
    Returns:
        np.array: Smoothed force time series.
    """
    fext_x = np.array(fext_x)
    t = np.arange(len(fext_x)) / fs  # Time vector
    
    # Find indices for the transition window
    start_idx = int(t_start * fs)
    end_idx = int(t_end * fs)
    
    # Sigmoid-based transition
    transition_length = end_idx - start_idx
    x_sigmoid = np.linspace(-5, 5, transition_length)  # Scaled sigmoid input
    smooth_curve = 1 / (1 + np.exp(-x_sigmoid))  # Sigmoid function
    
    # Scale sigmoid curve to smoothly adjust the force
    f_original = fext_x[start_idx:end_idx]
    f_smooth = f_original + (target_force - f_original) * smooth_curve
    
    # Apply the smooth transition
    fext_x[start_idx:end_idx] = f_smooth
    
    return fext_x

forcesmooth(fext_x,51,60,200)

def makezeros(rawlist, fs = 200, t_start = 50, t_end = 60):
    for i in range(t_start*fs,t_end*fs):
        rawlist[i] = 0
