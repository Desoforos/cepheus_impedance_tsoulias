...
def calculate_derivative(x, dt):
    """
    Calculate the time derivative (xdot) of a list of values x.
    
    Parameters:
        x (list of float): Input time series values.
        dt (float): Time step between samples.
        
    Returns:
        list of float: The derivative (xdot) of the input list.
    """
    if len(x) < 2:
        raise ValueError("Input list must have at least two elements to calculate a derivative.")
    
    xdot = [(x[i] - x[i - 1]) / dt for i in range(1, len(x))]
    xdot.insert(0, 0)  # Assume the initial derivative is 0 (optional)
    
    return xdot

def moving_median_filter(data, window_size):
    """
    Apply a moving median filter to a list of data.

    Parameters:
        data (list): The input data list to be filtered.
    ...
    for i, value in enumerate(data):
        ...
        # Remove oldest value if the window exceeds the window size
...
# xdot_filtered= moving_median_filter(xee_x_dot, 101)
xdotcopy = xee_x_dot.copy()
# xdot_filtered= moving_median_filter(xdotcopy,69)
xdot_filtered= moving_average_filter(xdotcopy,69)
# xdot_filtered = calculate_derivative(xfiltered, 0.005)

ydotcopy = xee_y_dot.copy()
# ydot_filtered = moving_median_filter(ydotcopy,69)
ydot_filtered = moving_average_filter(ydotcopy,69)
# ydot_filtered = calculate_derivative(xfiltered, 0.005)
...
axs[1, 0].plot(time_stamps[:des_len], xd_x_dot[:des_len], label='Desired Xdot', color='g',linestyle='--')
axs[1, 0].plot(time_stamps[:des_len], xee_x_dot[:des_len], label='Actual Xdot', color='b')
axs[1, 0].grid()
axs[1, 0].set_title('Xdot ')
axs[1, 0].set_xlabel('Time [s]')
axs[1, 0].set_ylabel('X Velcotiy [m/sec]')
axs[1, 0].legend()

# Plot Y values (Target, Desired, Actual)
axs[0, 1].plot(time_stamps[:des_len], xt_x[:des_len], label='Target X', color='r')
axs[0, 1].plot(time_stamps[:des_len], xd_x[:des_len], label='Desired X', color='g',linestyle='--')
# axs[0, 1].plot(time_stamps[:des_len], xfiltered[:des_len], label='Filtered X', color='b')
axs[0, 1].grid()
...
axs[1, 1].plot(time_stamps[:des_len], xd_x_dot[:des_len], label='Desired Xdot', color='g',linestyle='--')
axs[1, 1].plot(time_stamps[:des_len], xdot_filtered[:des_len], label='Filtered Xdot', color='b')
axs[1, 1].grid()
...
axs[1, 0].plot(time_stamps[:des_len], xd_x_dot[:des_len], label='Desired Xdot', color='g',linestyle='--')
axs[1, 0].plot(time_stamps[:des_len], xdot_filtered[:des_len], label='Filtered Xdot', color='b')
axs[1, 0].grid()
axs[1, 0].set_title('Xdot filtered')
axs[1, 0].set_xlabel('Time [s]')
...
axs[1, 1].plot(time_stamps[:des_len], xd_y_dot[:des_len], label='Desired Ydot', color='g',linestyle='--')
axs[1, 1].plot(time_stamps[:des_len], ydot_filtered[:des_len], label='Filtered Ydot', color='b')
axs[1, 1].grid()
axs[1, 1].set_title('Ydot filtered')
axs[1, 1].set_xlabel('Time [s]')
...
y = moving_average_filter(xee_y,10)
plt.plot(time_stamps[:len(y)],y, color = "b" , label="filtered y")
plt.plot(time_stamps[:len(y)],xd_y, color = "g", linestyle="--", label = "desired y")
plt.plot(time_stamps[:len(y)],xee_y, color = "r", label="actual y")
plt.grid()
plt.legend()
plt.title("filtered y with 3 point median")
plt.show()